...
openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /etc/nginx/ssl/prod.todora.dev.key \
    -out /etc/nginx/ssl/prod.todora.dev.crt \
    -subj "/C=PL/ST=Masovian/L=Warsaw/O=PK Ltd/OU=IT Department/CN=prod.todora.dev" > /dev/null 2>&1 &&
